...
- Persisted each market visit's sold cards, unique treasure and chapter-specific trade in the run save, closing refresh/recovery exploits. Event, rest, training and market exits now use single-settlement locks so rapid taps cannot duplicate rewards or skip multiple route layers.
- Corrected combat-zone card semantics: cleansing cards now find and purge Fate Curses across the hand, discard pile and draw pile; curses remain unplayable even with excess qi; and alchemy recycling only returns qi for cards actually recovered. Added deterministic unit and real-browser regression checks for cleanse, fallback and unplayable states.
- Split Artificer devices into typed Copper Finch and Thunder Hub instances. Draw, end-turn, reset, dismantle, fortress and Creation triggers now follow the printed card semantics, legacy saves migrate deterministically, and the mobile rail exposes cunning plus both device counts without hiding consumables.
- Completed Beastmaster state semantics: Fox scouting now pulls instruction/cooperation cards from the draw pile, Recall clears the active beast while preserving contracts, and Blood Moon Stampede reads the actual trial moon phase. The mobile profession badge exposes active beast, contract count and moon phase together.
- Aligned Sword Cultivator momentum with printed rules: attacks no longer gain an undocumented extra edge, while Jade Moon Bulwark draws only under Frost Moon. The mobile resource badge now pairs current edge with the live moon phase so conditional cards are readable before tapping.
- Aligned Alchemist harmonization with printed rules: Empty Cauldron Warming is disabled until both cold and heat are available, successful warming consumes one of each and grants its stated draw/shield package, and Yin-Yang Great Return Pill no longer adds hidden cold/heat damage or healing multipliers. Added four isolated-browser interaction regressions.
- Aligned Talisman Master sequencing with printed rules: Thunder Edict is disabled without seals, Thunder-Fire Chain counts its own spell segment, Wind-Borrowing Paper doubles Infernal Talisman's immediate burn settlement, and Red-Script Chain rewards qi from the card actually drawn after any reshuffle. The mobile profession badge now exposes seals, current-turn spell chain count and active burn amplification.
- Expanded the campaign from five to six complete chapters with “月沉归墟”, raising opening narrative volume from 15 to 30 scenes and adding two consequential opening decisions per chapter. Route-story nodes now use their own character scenes instead of repeating the chapter event.
- Upgraded all six bosses from a single three-move loop to two health-driven phases with unique transition dialogue, armor gain and a second three-move cycle. Mobile combat exposes the current phase without obscuring the hand, intent or end-turn control.
- Closed the narrative consequence loop with 12 collectible character epilogues. Each chapter's second major decision now changes the final character outcome, is recorded in the latest run, persists in the codex, and appears as a dedicated mobile chapter-summary card.
- Aligned Soul Contractor recursion with printed rules: empty recursion/copy/finisher cards are disabled, Impermanence Reaping prioritizes curses, Yellow Springs only removes cards that fit in hand, and Borrowed-Life Lamp immunity is explicitly tied to currently lit lamps. The mobile resource badge now exposes both lamps and unique discard memories.
